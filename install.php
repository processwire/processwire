...
class Installer {
	...
	protected function compatibilityCheck() { 
		...
		if($memoryLimit < 1) {
			$this->ok("PHP memory_limit is unlimited");
	...
	protected function profileImport($database, array $options) {
		...
		if(self::REPLACE_DB || !$result || $query->rowCount() == 0) {
			...
			$profileSql = $profile . 'install.sql';
			$coreSql = $profile . 'core-install.sql';
			
			if(!is_file($coreSql)) {
				if(file_put_contents($coreSql, ProcessWireCoreInstallSql()) === false) {
					$this->alertErr("Could not write $coreSql — check directory permissions");
					return;
				}
			}

			$this->profileImportSQL($database, $coreSql, $profileSql, $options); 
	...
	protected function adminAccountSave($wire) {
		...
		if(!self::TEST_MODE) {
			file_put_contents("./site/assets/installed.php", 
	...
	public function isInstalled() {
		if(Installer::TEST_MODE) return false; 
		return is_file("./site/assets/installed.php");
	}
...
class InstallerCli extends Installer {
	...
	public function execute() {
		...
		if($this->isInstalled()) {
			echo 
				"\nThe installer has already finished. Remove installer and related files with:\n" . 
				"\n    php install.php --cleanup\n\n";
			exit(0);
		}
...
HELP;
	...
	protected function runCleanup() {
		$optionalFiles = []; // items that the user may want to remove but we won't
		$this->h("Cleanup");
		if(!$this->isInstalled()) {
			$this->abort("No completed installation found. Run the installer first.");
		}
		$items = $this->getRemoveableItems();
		// Leave unused site-* profile directories and .gitignore alone — they may not belong to ProcessWire
		foreach(array_keys($items) as $key) {
			if(strpos($key, 'site-') === 0 || $key === 'gitignore') {
				$optionalFiles[$key] = $items[$key]['file'];
				unset($items[$key]);
			}
		}
		...
		if(is_file('./install-config.php')) {
			$items['install-config'] = array(
				...
				'path'  => __DIR__ . '/install-config.php',
		...
		if(count($optionalFiles)) {
			foreach($optionalFiles as $file) {
				$this->you("Remove if no longer needed: $file"); 
			}
		}
	}
...
PHP;
		if(file_put_contents($file, $content)) {
			$this->ok("Generated $file - please edit this file"); 
			$this->you("Fill in the required values and run: php install.php --config install-config.php");
		} else {
			$this->err("Could not write $file — check directory permissions"); 
			exit(1);
		}
	}
	...
	public function you($str) {
		echo " [YOU] " . $this->cleanStr($str) . "\n";
		return true;
	}
...
/**
 * Core starting SQL (which will merge with site profile SQL)
 * 
 * @return string
 * 
 */
function ProcessWireCoreInstallSql() {
	return <<<SQL
