...
class Pageimage extends Pagefile {
		...
		if($checkImage) {
			if($this->ext == 'svg') {
				...
				if($xml) {
					...
					$this->imageInfo['width'] = (int) $a->width > 0 ? (int) $a->width : $this->imagickSvgSize($this->filename, 'width');
					$this->imageInfo['height'] = (int) $a->height > 0 ? (int) $a->height : $this->imagickSvgSize($this->filename, 'height');
				}
			} else {
	...
	/**
	 * Gets the image size with PHP’s imagick extension if available, otherwise returns 100%
	 *
	 * #pw-internal
	 *
	 ...
	 *
	 */
	protected function imagickSvgSize($filename, $dimension) {
