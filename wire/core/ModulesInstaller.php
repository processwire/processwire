	...
	public function isUninstallable($class, $returnReason = false) {
		...
		if(!$reason) {
			...
			if(!empty($info['permanent'])) {
				...
				if(count($dependents)) {
					$reason = sprintf(
						$this->_("Module is required by other modules that must be removed first: %s"),
						implode(', ', $dependents)
					);
				}
			}
			...
			if(!$reason && in_array('Fieldtype', wireClassParents($namespace . $class))) {
				foreach($this->wire()->fields as $field) {
					...
					if($fieldtype == $class) {
						$reason = sprintf(
							$this->_("This module is a Fieldtype currently in use by one or more fields including: %s"),
							$field->name
						);
						break;
					}
