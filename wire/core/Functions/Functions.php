		...
		if(strpos($class, '=')) {
			...
			if(preg_match_all($re, $class, $matches)) {
				foreach($matches[1] as $key => $attrName) {
					...
					$attrVal = wire()->sanitizer->entities($attrVal);
...
/**
 * Get icon markup using Font Awesome 4 conventions (`fa` prefix, no brand/outline detection)
 *
 * Called by wireIconMarkup() when AdminThemeUikit is not active.
 *
 * #pw-internal
 *
...
/**
 * Get icon markup using Font Awesome 6 conventions (`fas`/`far`/`fab` prefix auto-detection)
 *
 * Called by wireIconMarkup() when AdminThemeUikit is active.
 *
 * #pw-internal
 *
 * @param string $icon Icon name with `fa-` prefix
...
function _wireIconMarkupFA6($icon, $class, array $attrs = array()) {
	...
	// Determine FA6 prefix: check renames first, then brands, then -o suffix
	$prefix = null;
	if(isset($renamed[$icon])) {
		$r = $renamed[$icon];
		if(is_array($r)) {
			list($icon, $prefix) = $r;
		} else {
	...
	if($prefix === null) {
		$iconName = substr($icon, 3); // strip 'fa-'
		if(isset($brands[$iconName])) {
			$prefix = 'fab';
		} else if(strlen($icon) > 5 && substr($icon, -2) === '-o') {
