...
class PageFinder2 extends Wire {
	...
	static protected $defaultOptions = [
		...
		/**
		 * Use nested subqueries for subselectors (rather than separate queries)
		 * 
		 * (so far unclear if this is an improvement)
		 * 
		 * @since 3.0.257
		 * 
		 */
		...
		/**
		 * Allow PageFinder instances to be reused rather than creating a new one each time?
		 * 
		 * @since 3.0.257
		 * 
		 */	
		...
		/**
		 * Populate this query rather than creating a new one
		 * 
		 */	
		'query' => null, 
	...
	/**
	 * Is the PagePaths module installed?
	 * 
	 * @var null 
	 * 
	 */
	protected $hasPagePaths = null;
	
	/**
	 * PageFinder instance pool
	 * 
	 * @var PageFinder[]
	 * 
	 */
	 ...
	 * 
	 */
	static protected $singlesFields = [
		'has_parent' => 1,
		'hasParent' => 1,
		'num_children' => 1,
		'numChildren' => 1,
	...
	static protected $reverseDbOperators = [
		'=' => '!=',
		'!=' => '=',
		'<' => '>=',
		'<=' => '>',
	...
	static protected $modifierFields = [
		'include' => 'include',
		'_custom' => '_custom',
		'limit' => 'limit',
		'start' => 'start',
	...
	static protected $modifierAlternates = [
	 ...
	 *
	 */ 
	static protected $noArrayFields = [
	...
	protected function init(Selectors $selectors, array $options) {
		...
		if(!$this->config || $this->config !== $this->wire()->config) {
			$names = ['fields', 'pages', 'sanitizer', 'database', 'languages', 'templates', 'config'];
		...
		$this->start = 0;
		$this->includeMode = '';
		$this->extraOrSelectors = [];
		$this->sortsAfter = [];
		$this->reverseAfter = false;
		$this->finalSelectors = null;
		...
		$options = array_merge(self::$defaultOptions, $this->config->PageFinder, $options);
		...
		foreach($selectors as $key => $selector) {
			...
			if(is_array($fieldName)) {
				foreach($fieldName as $name) {
					if(isset(self::$noArrayFields[$name])) $disallow = "field:$name";
					if($disallow) break;
				}
				$fieldName = $selector->field(); // force string
			} else if(isset(self::$noArrayFields[$fieldName]) && is_array($value)) {
				if(self::$noArrayFields[$fieldName] > 1) $disallow = 'value';
			}
	...
	public function ___find($selectors, array $options = array()) {
		...
		$timer = empty($options['testMode']) || self::$level > 1 ? null : Debug::startTimer($this->className());
		...
		if($options['loadPages'] || $this->getTotalType === 'calc') {
			...
			foreach($query->orderby as $f) {
				if(strpos($f, '_score_') !== 0) continue;
				$scoreField = $f;
				if(strpos($scoreField, ' ')) list($scoreField,) = explode(' ', $scoreField, 2);
				$scoreFields[$scoreField] = $scoreField;
			}
				...
				while($row = $stmt->fetch(\PDO::FETCH_ASSOC)) {
					...
					if($options['returnVerbose']) {
						...
						foreach($scoreFields as $scoreField) {
							if(!isset($row[$scoreField])) continue;
							$v = (float) $row[$scoreField];
							if($v === 111.1 || $v === 222.2 || $v === 333.3) {
								// signal scores of non-match
							} else {
								$score += $v;
								unset($row[$scoreField]);
							}
						}
						$row['score'] = $score;
						$matches[] = $row;
	...
	public function findVerboseIDs($selectors, $options = array()) {
		...
		$options['returnAllColsOptions'] = self::$defaultOptions['returnAllColsOptions'];
		if($hasCustomOptions) {
	...
	protected function preProcessSelectors(Selectors $selectors, $options = array()) {
		...
		if($start !== null && $start < 0) {
			...
			if($limit) {
				...
				$finder = self::getInstance($this); // new PageFinder
				$this->wire($finder);
				$finder->find($sel);
	...
	protected function preProcessSelector(Selector $selector, Selectors $selectors, array $options, $level = 0) {
		...
		if($quote == '[') {
			...
			$this->preProcessSubSelector($selector, $selectors, $options);
	...
	*/
	 ...
	 * 
	 */
	protected function preProcessSubSelector(Selector $selector, Selectors $parentSelectors, array $options) {
		...
		$pageFinder = self::getInstance($this); // $this->wire(new PageFinder());
		
		if(!empty($options['useSubQueries']) && count($selector->fields()) === 1) {
			...
			if($field && $field->type instanceof FieldtypePage) {
				...
				$this->wire($query);
				$query->bindOption('global', true); // ensures bound value key are globally unique
				$query->bindOption('prefix', 'pfsq');
				$query = $pageFinder->find($selectors, ['returnQuery' => true, 'query' => $query]);
				$selector->set('_pageFinderQuery', $query);
				return;
			}
		}
		
		// we will locate pages to match in a separate query:
		...
		if(count($ids) == 0) {
			...
			foreach($selector->fields as $key => $fieldName) {
				...
				$field = $this->isPageField($fieldName);
				if(is_string($field) && isset(self::$natives[$field])) {
		...
		} else if(isset(self::$natives[$fieldName])) {	
	...
	protected function ___getQuery($selectors, array $options) {
		...
		$query = empty($options['query']) ? $this->wire(new DatabaseQuerySelect()) : $options['query'];
		
		if(!empty($options['bindOptions'])) {
			foreach($options['bindOptions'] as $k => $v) {
				$query->bindOption($k, $v);
			}
		}
		...
		foreach($selectors as $selector) {
			...
			if($field1 === 'sort' && $selector->operator === '=') {
				...
				if(!isset(self::$dbOperators[$selector->operator])) {
			...
			foreach($fields as $fieldName) {
				...
				if($selector->quote === '[' && !empty($options['useSubQueries']) && count($fields) === 1) {
					if($this->getSubQuery($query, $selector, $field, $subfield, $tableAlias)) continue;
				}
				...
				foreach($valueArray as $value) {
					...
					if($useEmpty && strpos($subfield, 'data') === 0) { // && !$fieldtype instanceof FieldtypeMulti) {
						...
						if(isset(self::$dbOperators[$operator])) {
	...
	/**
	 * Populate a subquery found during pre-processing into the main query
	 * 
	 * Used only if the useSubQueries option is true. 
	 * 
	 ...
	 * 
	 */
	protected function getSubQuery($query, $selector, $field, $subfield, $tableAlias) {
		
		$q = $selector->get('_pageFinderQuery'); /** @var DatabaseQuerySelect $q */
		if(!$q) return false;
		
		$operator = $selector->operator; 
		
		if($operator != '=' && $operator != '!=') return false;
		if(strpos($subfield, '.')) return false;
		if(!$field instanceof Field) return false;
		if(!$field->type instanceof FieldtypePage) return false;
		...
		$join = "$table AS $tableAlias ON ($tableAlias.pages_id=pages.id AND $tableAlias.$subfield $in(\n$sql\n))";
		$query->join($join);
	...
	protected function whereEmptyValuePossible(Field $field, $col, $selector, $query, $value, &$where) {
		...
		$blankValue = $ft->getBlankValue($this->nullPage(), $field);
		$blankIsObject = is_object($blankValue); 
		$whereType = 'OR';
		$sql = '';
		
		if($blankIsObject) $blankValue = '';
		if(!isset(self::$reverseDbOperators[$operator])) return false; 
		if($selector->not) $operator = self::$reverseDbOperators[$operator]; // reverse
	...
	protected function getQueryAllowedTemplates(DatabaseQuerySelect $query, $options) {
		...
		static $cacheWireInstanceID = null;
		
		$wireInstanceID = $this->wire()->getProcessWireInstanceID();
		
		if($cacheUserID !== $user->id || $cacheWireInstanceID !== $wireInstanceID) {
			// clear cached values
			$where = null;
			$where2 = null;
			$leftjoin = null;
			$cacheUserID = $user->id;
			$cacheWireInstanceID = $wireInstanceID;
		}
		...
		if($user->isGuest()) {
			...
			foreach($this->templates as $template) {
				...
				foreach($template->roles as $role) {
					if($role->id != $guestRoleID && !isset($userRoleIDs[$role->id])) continue;
					$yesTemplates[$template->id] = $template;
					break;
				}
			}
		...
		if($noCnt) {
			...
			$leftjoin = 
				...
				implode(',', array_keys($noTemplates)) . '))';
		...
		if($noCnt > 0 && $noCnt < $yesCnt) {
			$in = implode(',', array_keys($noTemplates));
			$yes = false; 
		} else {
			$in = implode(',', array_keys($yesTemplates));
			$yes = true;
		}
	...
	protected function getQuerySortSelector(DatabaseQuerySelect $query, Selector $selector) {
		...
		foreach($values as $value) {
			...
			if($value == 'random') { 
			...
			} else if(($value === 'path' || $value === 'url') && $this->hasPagePaths()) {
				static $pathN = 0;
				$pathN++;
				...
				if($field->type instanceof FieldtypePage) {
					...
					$blankValue = $field->type->getBlankValue($this->nullPage(), $field);
				}
	...
	protected function getQueryNativeField(DatabaseQuerySelect $query, $selector, $fields, array $options, $selectors) {
		...
		/*
		if($selector->quote === '[' && count($fields) === 1) {
			$field = reset($fields);
			if($this->getSubQuery($query, $selector, $field, '', '')) return;
		}
		*/
		...
		foreach($fields as $field) { 
			...
			if($isParent || $isChildren || $isPages) {
				...
				if(($isPages || $isParent) && !$isPartialOperator 
					&& (!$subfield || $subfield === 'id' || $subfield === 'path' || $subfield === 'url')) {
					...
					if(!$this->fields->isNative($subfield)) {
						$finder = self::getInstance($this); // $this->wire(new PageFinder());
			...
			if(count($IDs)) {
				...
				if($field === 'templates_id' || $field === 'template') {
				...
				} else if($field === 'created' || $field === 'modified' || $field === 'published') {
				...
				} else if($field === 'id' || $field === 'parent_id' || $field === 'sort') {
					$value = (int) $value; 
				}
	...
	protected function getQueryNumChildren(DatabaseQuerySelect $query, $selector) {
		...
		if(!isset(self::$dbOperators[$op])) {
			$this->syntaxError("Operator '$op' not allowed for 'num_children' selector.");
		}
	...
	protected function arrangeFields(array $fields) {
		...
		foreach($fields as $name) {
			...
			if(isset(self::$singlesFields[$name])) {	
				$singles[] = $name;
			}
		}
	...
	protected function isPageField($fieldName, $literal = false) {
		...
		if($field) {
			if($field->type instanceof FieldtypePage) {
				...
				$test = $field->type->getBlankValue($this->nullPage(), $field); 
	...
	protected function isModifierField($name) {
		if(isset(self::$modifierAlternates[$name])) return self::$modifierAlternates[$name];
		if(isset(self::$modifierFields[$name])) return self::$modifierFields[$name];
		return false;
	}
	...
	protected function hasNativeFieldName($fieldNames) {
		...
		foreach($fieldNames as $name) {
			...
			if($name === 'parent' || $name === 'children' || $name === 'child') {
				$hasNative = true;
				break;
			}
		}
	...
	/**
	 * Is this instance finished with its current job?
	 * 
	 * #pw-internal
	 * 
	 * @param bool|null $finished Optionally set finished state
	 * @return bool
	 * @since 3.0.257
	 * 
	 */
	public function isFinished($finished = null) {
		if(is_bool($finished)) $this->finished = $finished;
	 ...
	 * 
	 */
	protected function nullPage() {
		if($this->nullPage === null) $this->nullPage = $this->pages->newNullPage();
	 ...
	 * 
	 */
	protected function hasPagePaths() {
		if($this->hasPagePaths === null) {
			$this->hasPagePaths = $this->wire()->modules->isInstalled('PagePaths');
		}
		return $this->hasPagePaths;
	}
	
	/**
	 * Get PageFinder instance
	 * 
	 * @return PageFinder2
	 * @param Wire $wire
	 * @param bool $forceNew Require it to be a new instance?
	 * 
	 * #pw-internal
	 * 
	 */
	public static function getInstance(Wire $wire, $forceNew = false) {
		static $settings = null;
		if($settings === null) $settings = $wire->wire()->config->PageFinder;
		if($forceNew || (isset($settings['reuseInstances']) && !$settings['reuseInstances'])) {
			$pageFinder = new PageFinder2();
			$wire->wire($pageFinder);
			return $pageFinder;
		}
		$pageFinder = null;
		foreach(self::$instances as $instance) {
			/** @var PageFinder2 $instance */
			if($instance->isFinished() && $instance !== $wire && $instance->wire() === $wire->wire()) {
				$pageFinder = $instance;
				break;
			}
		}
		if($pageFinder) {
			...
			$pageFinder = new PageFinder2();
			$wire->wire($pageFinder);
			$instanceNum = $pageFinder->getInstanceNum();
			self::$instances[$instanceNum] = $pageFinder;
		}
		return $pageFinder;
	}
