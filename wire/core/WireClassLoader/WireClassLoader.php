...
class WireClassLoader {
	...
	/**
	 * Quick lookup index to determine if $classMaps should be iterated
	 * 
	 * @var array 
	 * 
	 */
	protected $classMapsIndex = [];
	
	/**
	 * @var int 
	 * 
	 */
	protected $numClassMapPaths = 0;
	 ...
	 * 
	 * If the file basename is omitted and file ends with a slash, 
	 * then the ClassName + '.php' is assumed. 
	 * 
	 * @param array $classMap Array of class files indexed by class name. Files must have no leading slash.
	 * @param string $fromPath Starting path (with trailing slash)
	 * @since 3.0.260
	 * 
	 */
	public function addClassMap(array $classMap, $fromPath) {
		if(!isset($this->classMaps[$fromPath])) $this->classMaps[$fromPath] = [];
		$this->numClassMapPaths++;
		foreach($classMap as $class => $file) {
			$this->classMaps[$fromPath][$class] = $file;
			$this->classMapsIndex[$class] = true; // lookup index
		}
	}
	...
	public function loadClass($className) {
		...
		if($namespace === 'ProcessWire' && isset($this->classMapsIndex[$name])) {
			$file = '';
			$fromPath = '';
			if($this->numClassMapPaths === 1) {
				$fromPath = key($this->classMaps);
				$file = $this->classMaps[$fromPath][$name];
			} else {
				foreach(array_keys($this->classMaps) as $fromPath) {
					if(!isset($this->classMaps[$fromPath][$name])) continue;
					$file = $this->classMaps[$fromPath][$name];
					break;
				}
			}
