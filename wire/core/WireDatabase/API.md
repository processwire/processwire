...
- **Settings:** `$config->dbOptions['pgsql']` may contain `schema` (a schema other than `public`, added to
  ...
  skipped rather than failing), `savepoints`, `fold` and `fulltext` (see the table below). Assign the whole array, as in
